...
public class AuthServiceBKP 
{
	...
	public void unlinkCredential(int personId, int credentialId)
	{
		String hql="";
		hql+="FROM AuthCredential ";
		hql+="WHERE credentialId=:cid ";
		hql+="  AND person.personId=:pid ";
		hql+="  AND deletedAt IS NULL ";

		AuthCredential cred = dao.querySingleRow(hql,"cid",credentialId,"pid",personId);

		if(cred==null) return;

		cred.setDeletedAt(new Timestamp(System.currentTimeMillis()));
	}
